...
def make_sample_her_transitions_obj_policy(replay_strategy, replay_k, reward_fun, obj_rnd, obj_normalizer):
    """Creates a sample function that can be used for HER experience replay.
    ...
    def _sample_her_transitions(episode_batch, batch_size_in_transitions):
        ...
        obj_rnd[0].cuda()
        obj_rnd[1].cuda()
        rnd_input = K.zeros(batch_size, obj_normalizer.get_stats()[0].shape[0], dtype=K.float32, device='cuda')
        rnd_input[:,-transitions['ag_2'].shape[1]::] = K.tensor(transitions['ag_2'], dtype=K.float32, device='cuda')
        rnd_input = obj_normalizer.preprocess(rnd_input)
        rnd_input = rnd_input[:,-transitions['ag_2'].shape[1]::]
        with K.no_grad():
            transitions['obj_rnd'] = F.mse_loss(obj_rnd[0](rnd_input), obj_rnd[1](rnd_input), reduction='none').mean(1, keepdim=True)
